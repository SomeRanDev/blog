...
Please note at the time of writing this, Haxe 4.3 is the latest release and what this article is using. While unlikely, future releases may generate C++ differently, so keep that in mind future people reading this!
...
### Using Value-Types Without Using @:nativeGen
* [What Does the C++ Look Like?](/blog/haxe/hxcpp_no_gc_tips#what-does-the-c-look-like)
...
@:nativeGen
```
This metadata removes all reflection capability from a Haxe class, but the C++ output for the class becomes significantly cleaner.
...
# What's a Value-Type?
Value-types are types generated on the stack in C++. For example:
```cpp
MyClass obj; // value-type
...
Instead of using a garbage collector, C++ will copy around objects. This can be better for performance when generating tons of objects frequently since the creation/destruction occurs consistently instead of building up lag spikes when the gc clears.
...
`Dynamic` is implemented in the hxcpp library and works just like how it does in Haxe. It's meant to store any object and use reflection to access its properties. However, our value-type object does not have access to reflection capabilities (and also `Dynamic` is slow), so we need to avoid it!
